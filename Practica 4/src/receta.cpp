...
void receta::setNombre(string nuevo_nombre){
    nombre=nuevo_nombre;
}

void receta::setCode(string nuevo_code){
    code = nuevo_code;
}

void receta::setPlato(unsigned int num_plato){
    plato = num_plato;
}

void receta::setIngredientes(list<pair<string,unsigned int>> lista){
    ings = lista;
}

void receta::fusionarRecetas(receta r1, receta r2){
    receta aux;
    bool enc = false;

    list<pair<string, unsigned int>> lista, lista_aux;
    list<pair<string, unsigned int>>::iterator it1, it2;

    string nc = "F_" + r1.getCode() +"_" + r2.getCode();
    string nn = "Fusion de "+r1.getNombre() + " y " + r2.getNombre();
    aux.setCode(nc);
    aux.setNombre(nn);
    aux.setPlato(r1.getPlato());

    lista = r1.getIngs();
    lista_aux = r2.getIngs();
    
    it1 = lista.begin();
    it2 = lista_aux.begin();
    
    while(it2 != lista_aux.end()){
        while(it1!=lista.end() && !enc){
    ...
    aux.setIngredientes(lista);

    *this = aux;
}

bool receta::esValido(Ingredientes otros){
    receta::iterator it = this->begin();
    Ingredientes::iterator it2 = otros.begin();
    bool enc = false, fin = false;

    while(it != this->end() && !fin){
        while(it2 != otros.end() && !enc){
